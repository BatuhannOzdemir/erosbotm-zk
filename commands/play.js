...
module.exports = {
  ...
  cooldown: 3,
  aliases: ["p"],
  description: "Şarkı Oynatmanızı Sağlar.",
  async execute(message, args) {
    ...
    if (!args.length)
      return message
        .reply(`Kullanım: ${message.client.prefix}play <YouTube URL | Video Adı | Soundcloud URL>`)
        .catch(console.error);

    const permissions = channel.permissionsFor(message.client.user);
    if (!permissions.has("CONNECT"))
      return message.reply("Odaya Bağlanma Yetkisine Sahip Değilim!");
    if (!permissions.has("SPEAK"))
      return message.reply("Konuşma Yetkisine Sahip Değilim!");
